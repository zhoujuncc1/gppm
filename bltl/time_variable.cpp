...
std::unordered_set<std::string> pool=std::unordered_set<std::string>();
TimeVariable::TimeVariable() :
		name(get_name()), value(10000000) {
}

TimeVariable::TimeVariable(std::string str) {
	size_t equal = str.find('=');
	if (equal != std::string::npos) {
		name = str.substr(1, equal);
		value = std::stoi(str.substr(equal + 1, str.length() - 1));
		isfix=true;
	} else if ('0' < str[1] < '9') {
		name = get_name();
		value = std::stoi(str);
