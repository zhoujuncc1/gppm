...
class AtomicNode : public Node {
	public:
		AtomicNode(int v, char s, int l) : varID(v), sign(s), level(l) {}
		void update(int* levels, int isLast){
			//printf("--> Update atomic node\n");
			this->known = 1;
				if (sign=='=')
				{
					if(levels[varID-1]==level)
						this->truth = 1;
					else
						this->truth = 0;
				}
		...
		Node* duplicate(){
			Node* copyNode = new AtomicNode(varID,sign,level);
			if(child1)
				copyNode->setChild1(child1->duplicate());
			if(child2)
				copyNode->setChild2(child2->duplicate());
