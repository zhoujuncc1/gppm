...
void generate_time(map<string, TimeVariable*> params, double step_size,
	...
	for (auto itr = params.begin(); itr != params.end(); itr++) {
		...
		int left_bound = current[itr->first]
				- (current[itr->first] - itr->second->range.first) * step_size;
		int right_bound = current[itr->first]
				+ (itr->second->range.second - current[itr->first]) * step_size;
		std::uniform_int_distribution<int> distribution(left_bound,
				right_bound);
...
double _recursive_generate_prd(Parameter* param, map<string, double> &value_set,
	...
	if (initial_state.find(param->name) != initial_state.end()) {
		...
		double min = param->range.second;
		for (int i = 0; i < param->children.size(); i++) {
			double value = _recursive_generate_prd(param->children[i],
					value_set, initial_state);
			if (min > value)
				min = value;
		}
		if (param->range.first > min)
...
double _recursive_generate_prd(Parameter* param, map<string, double> &value_set,
	...
	if (param->range.first > min)
		throw -1;
	double left_bound = current[param->name]
			- (current[param->name] - param->range.first) * step_size;
	double right_bound = current[param->name]
			+ (param->range.second - current[param->name]) * step_size;
	if (right_bound > min)
		right_bound = min;
