...
__global__ void eval_prd_kernel(float *traj, int *value, int var_id, float left, float right, int *dim_array_device)
{
	int n = threadIdx.x;
    int t = threadIdx.y;
...
__global__ void eval_G_kernel(int* value_arr, int* sub_value_arr, int distance, int *dim_array_device){
	int n = threadIdx.x;
    int t = threadIdx.y;
...
__global__ void eval_F_kernel(int* value_arr, int* sub_value_arr, int distance, int *dim_array_device){
	int n = threadIdx.x;
    int t = threadIdx.y;
    int index = n*dim_array_device[TRAJ_END_TIME_I]+t;
	value_arr[t] = false;
...
bool* GPUBltlChecker::eval_bltl_recursive(Bltl *bltl)
{
    if(bltl->getOperation() == op_PRD)
        return bltl->prd->value_array_device;
	else
	{
        bool* sub_value_arr = eval_bltl(bltl->getChild1());
		error = cudaMalloc((void **)&(bltl->value_array_device), traj_provider->dim_array[TRAJ_SIZE_I]*length * sizeof(bool));
...
int GPUBltlChecker::eval_bltl(Bltl *bltl){
    eval_bltl_recursive(bltl);
    bool* result_arr = (bool*) malloc(traj_provider->dim_array[TRAJ_SIZE_I]*length)

    error = cudaMemcpy(result_arr, bltl->value_array_device, traj_provider->dim_array[TRAJ_SIZE_I]*length * sizeof(bool), cudaMemcpyDeviceToHost);
