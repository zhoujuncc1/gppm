...
public:
    BltlChecker() {}
    BltlChecker(Bltl* bltl) { this->bltl = bltl; }
    virtual ~BltlChecker() {}
    virtual int check(Trajectory traj) = 0;
    virtual vector<int> check() { return vector<int>(); }

    virtual vector<int> check(vector<Trajectory> trajectories){return vector<int>();}
    Bltl* bltl;
    map<string, Prd*> prds;
    int isGPU;
};
...
public:
    ...
    RecursiveBltlChecker(Bltl* bltl, map<string, Prd*> prds, Trajectory traj)
        : BltlChecker(bltl)
    {
        isGPU = false;
        this->prds = prds;
    ...
    virtual vector<int> check(vector<Trajectory> trajectories)
    {
        long elapsed_secs = 0;
        vector<int> result;
        for (auto traj_itr = trajectories.begin(); traj_itr != trajectories.end();
             traj_itr++) {
            clock_t begin = clock();
            eval_prds(*traj_itr);
            clock_t end = clock();
            ...
            result.push_back(eval_formula(bltl, 0));
        }
        //printf("Prd Time: %ld\n", elapsed_secs);
        return result;
    }
...
public:
    GPUBltlChecker(Bltl* bltl, map<string, Prd*> prds,
