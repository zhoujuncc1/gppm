...
public:
    Miner() {}
    /*
      Miner(pt::ptree in, string bltl_input, vector<string> prd_inputs,
  ...
  */
    ...
    void refine_time()
    {
        for (auto itr = params->unknown_prd_set.begin();
             itr != params->unknown_prd_set.end(); itr++)
            itr->second->value = state->prd_values[itr->first];
        for (auto itr = params->unknown_time_set.begin();
             itr != params->unknown_time_set.end(); itr++)
        ...
        for (auto itr = params->unknown_time_set.begin();
            ...
            double best_loss = (*(state->loss))(state);
    ...
    string to_string()
    {
        std::stringstream ss;
        ss << "{";
        for (auto itr = params->known_prd_set.begin();
             itr != params->known_prd_set.end(); itr++)
            ss << itr->first << ": " << itr->second->value << ", ";
        for (auto itr = params->known_time_set.begin();
             itr != params->known_time_set.end(); itr++)
            ss << itr->first << ": " << itr->second->value << ", ";
        for (auto itr = params->unknown_prd_set.begin();
             itr != params->unknown_prd_set.end(); itr++)
...
public:
    SingleMiner() {}
    void mineGeneral() { do_mine(state, input.get_child("input.config")); }
};
...
public:
    static vector<Miner*> buildMiner(string filename, bool isFileMiner)
    {
        ...
        BOOST_FOREACH (pt::ptree::value_type& v,
            input.get_child("input.constraints")) {
            // The data function is used to access the data stored in a node.
        ...
        if (isFileMiner) {
            for (auto itr = bltl_inputs.begin(); itr != bltl_inputs.end(); itr++)
                miners.push_back(new FileMiner(input, *itr + ";", prd_inputs,
                    constraint_inputs, weight_inputs));
            return miners;
        }
    }
    static vector<Miner*> buildGPUMiner(string filename)
    {
        ...
        for (auto itr = bltl_inputs.begin(); itr != bltl_inputs.end(); itr++)
            miners.push_back(new GPUMiner(input, *itr + ";", prd_inputs,
                constraint_inputs, weight_inputs));
        return miners;
    }
};
